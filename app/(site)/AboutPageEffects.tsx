...
/**
 * Ports legacy about-page behaviour: cursor, mobile nav, hero stagger,
 * scroll reveals, progress bar, announce bar, navbar scroll state, floating CTA.
 */
export function AboutPageEffects() {
  useEffect(() => {
    ...
    animEl("hero-h1", 380);
    animEl("hero-desc", 520);
    animEl("hero-reg", 600);
    animEl("hero-actions", 700);
    animEl("hero-stat", 880);
    animEl("hero-caption", 300);
    ...
    const boostSel = [
      "a",
      "button",
      `.${CSS.escape(styles["about-value-card"])}`,
      `.${CSS.escape(styles["about-exp-tag"])}`,
      `.${CSS.escape(styles["about-svc-item"])}`,
      `.${CSS.escape(styles["about-exp-item"])}`,
    ].join(", ");
