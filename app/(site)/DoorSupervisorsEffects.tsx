...
/**
 * Door supervisors page: cursor, nav, hero stagger, scroll reveals,
 * progress bar, announce bar, floating CTA, clock readout.
 */
export function DoorSupervisorsEffects() {
  useEffect(() => {
    ...
    animEl("dh-h1", 380);
    animEl("dh-sub", 520);
    animEl("dh-actions", 660);
    animEl("dh-badge", 860);

    const clk = document.getElementById("ds-clock");
    ...
    const boostSel = [
      "a",
      "button",
      `.${CSS.escape(styles["ds-venue-card"])}`,
      `.${CSS.escape(styles["ds-duty"])}`,
      `.${CSS.escape(styles["ds-why-item"])}`,
