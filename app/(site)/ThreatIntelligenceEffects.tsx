...
/**
 * Threat intelligence page: cursor, nav, hero stagger, scroll reveals,
 * progress bar, announce bar, floating CTA, clock readout.
 */
export function ThreatIntelligenceEffects() {
  useEffect(() => {
    ...
    animEl("th-h1", 380);
    animEl("th-sub", 520);
    animEl("th-actions", 660);
    animEl("th-card", 860);

    const clk = document.getElementById("ti-clock");
    ...
    const boostSel = [
      "a",
      "button",
      `.${CSS.escape(styles["ti-cap-item"])}`,
      `.${CSS.escape(styles["ti-card"])}`,
      `.${CSS.escape(styles["ti-card-link"])}`,
      `.${CSS.escape(styles["ti-tag"])}`,
      `.${CSS.escape(styles["ti-step"])}`,
      `.${CSS.escape(styles["ti-overview-img"])}`,
