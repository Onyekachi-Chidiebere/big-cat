...
/**
 * Security guards page: cursor, nav, hero stagger, scroll reveals,
 * progress bar, announce bar, floating CTA, clock readout.
 */
export function SecurityGuardsEffects() {
  useEffect(() => {
    const fSel = [styles["f-reveal"], styles["f-reveal-x"], styles["f-reveal-l"], styles["f-reveal-scale"]]
      .filter(Boolean)
      .map((c) => `.${CSS.escape(c)}`)
      .join(", ");
    ...
    animEl("sh-h1", 380);
    animEl("sh-sub", 520);
    animEl("sh-actions", 660);
    animEl("sh-status", 860);

    const clk = document.getElementById("sg-clock");
    ...
    const boostSel = [
      "a",
      "button",
      `.${CSS.escape(styles["sg-card"])}`,
      `.${CSS.escape(styles["sg-other-card"])}`,
      `.${CSS.escape(styles["sg-ops-item"])}`,
      `.${CSS.escape(styles["btn-pill-white"])}`,
      `.${CSS.escape(styles["btn-pill"])}`,
      `.${CSS.escape(styles["btn-pill-ghost"])}`,
    ].join(", ");
