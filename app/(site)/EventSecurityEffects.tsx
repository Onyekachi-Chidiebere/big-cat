...
/**
 * Event security page: cursor, nav, hero stagger, scroll reveals,
 * progress bar, announce bar, floating CTA, clock readout.
 */
export function EventSecurityEffects() {
  useEffect(() => {
    ...
    const boostSel = [
      "a",
      "button",
      `.${CSS.escape(styles["es-card"])}`,
      `.${CSS.escape(styles["es-card-link"])}`,
      `.${CSS.escape(styles["es-type-item"])}`,
      `.${CSS.escape(styles["es-cap"])}`,
