...
/**
 * Ports legacy what-we-do inline script: cursor, mobile nav, hero stagger,
 * scroll reveals, progress bar, announce bar, navbar scroll state, floating CTA, clock.
 */
export function WhatWeDoEffects() {
  useEffect(() => {
    ...
    if (pgClock) {
      const tick = () => {
        const n = new Date();
        pgClock.textContent = [n.getHours(), n.getMinutes(), n.getSeconds()]
    ...
    animEl("hero-label", 200);
    animEl("hero-h1", 380);
    animEl("hero-p", 520);
    animEl("hero-actions", 640);
    animEl("hero-status", 820);
    animEl("hero-card", 960);
    ...
    const boostSel = [
      "a",
      "button",
      `.${CSS.escape(styles["wwd-svc-card"])}`,
      `.${CSS.escape(styles["wwd-strat-card"])}`,
      `.${CSS.escape(styles["wwd-tag"])}`,
    ].join(", ");
